...
class STTNode(Node):
    ...
    def _mic_loop(self):
        ...
        # )
        stream = pa.open( format=pyaudio.paInt16, channels=2, rate=self._rate, input=True, input_device_index=2, frames_per_buffer=self._vad.frame_bytes // 2, )
        try:
            while rclpy.ok():
                ...
                utterance = self._vad.process_frame(raw)
                if utterance:
